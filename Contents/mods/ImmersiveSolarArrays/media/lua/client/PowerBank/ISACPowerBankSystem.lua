...
CGlobalObjectSystem.RegisterSystemClass(ISACPowerBankSystem)
...
local function DoSpecialTooltip(tooltipUI, square) -- creates tooltip for amount of water in barrel we could do the same for power
	...
	local powerBank = ISACPowerBankSystem.instance:getIsoObjectOnSquare(square)
	if not powerBank or not powerBank:getModData()["powerMax"] then return end
